...
export default class Poster {
  ...
  constructor(config) {
    this.id = config.id
    this.ctx = Taro.createCanvasContext(config.id, config.replaceView)
  ...
  drawCycle = (img, x, y, radius) => {
    ...
    this.ctx.drawImage(img, imgX, imgY, (radius * this.scale * 2), (radius * this.scale * 2))
    this.ctx.restore()
    this.ctx.save()
  }
  ...
  draw = () => {
    ...
    this.items.forEach((item) => {
      if (item.type === 'text') {
        ...
        this.drawCycle(item.image, item.x, item.y, item.radius)
      }
    })
  ...
  generateTempImage = () => {
    ...
    Taro.canvasToTempFilePath({
      ...
      fail: function (res) {
        console.error('generate_temp_image_fail', res)
      }
    })
  }
